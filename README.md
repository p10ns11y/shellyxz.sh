...
- See [arch-design/VERIFICATION.md](arch-design/VERIFICATION.md) for agent workflow (`aw` focus + `av` verify cockpit, tmux Prefix+W/V, nvim Telescope keymaps, `gdf`/`gdfs`, delta via git include)
...
| `agent_verify` / `agent_work` refuses in Cursor | editor terminal guard | Use Ghostty/tmux (`t` or Super+Alt+Return); see [arch-design/VERIFICATION.md](arch-design/VERIFICATION.md) |
