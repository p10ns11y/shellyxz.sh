...
Run verification in **Ghostty + tmux** (`t` or Super+Alt+Return). **Planned:** `ts` — per-project tmux session ([coming-next.md](coming-next.md) SN-TS). Cursor integrated terminals skip `mise activate` and refuse `agent_build` / `agent_verify` (`ab` / `av`; `af`/`aw`/`agent_work` are legacy) (phantom-tab / no-cockpit UX).
